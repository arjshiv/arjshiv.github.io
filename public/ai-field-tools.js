(() => {
  const corpus = [
    {
      ...
      title: 'ResiDesk',
      href: '#now',
      text: 'I co-founded ResiDesk to help property teams turn resident texts, reviews, tickets, and calls into answers, history, and follow-up the team can act on without rereading the whole thread.',
    ...
    },
    {
      ...
      text: 'A prototype can win the room and still lose on the next morning. Busy use is the test.',
    ...
    },
    {
      ...
      title: 'AI view',
      href: '#writing',
      text: 'I care about AI that works on a normal Tuesday. History, evals, handoff, trust, and judgment matter more than model names.',
    ...
    },
    {
      ...
      title: 'BlackRock',
      href: '#about',
      text: 'At BlackRock, interface quality mattered because real money sat behind the decision. The tool had to survive the next morning when the real call came back.',
    ...
    },
    {
      ...
      title: 'Climb Credit',
      href: '#about',
      text: 'At Climb Credit, we asked what happened after the program. That pushed outcomes into underwriting, product, and data while annual loan volume moved from 1 million dollars to 300 million dollars and the work stayed close to the borrower.',
  ...
  const guides = {
    ...
    founder: [
      ['#work-table', 'Start with Climb, BlackRock, and the work that came after.'],
  ...
  const tuesdayEnvironments = {
    operator: 'a busy property team with too many small exceptions in a row',
  ...
  const tuesdayBreakpoints = {
    answers: ['missing policy or lease context', 'unclear handoff to the owner', 'tone misses the room'],
  ...
  const tweakBriefs = {
    legibility: {
      ...
      steps: ['Audit hover states first; text should never get lighter or less legible during motion.', 'Keep long paragraphs under 68 characters per line on desktop.', 'Use borders to group choices, not to outline every empty surface twice.'],
    },
    speed: {
      title: 'Speed fix',
    ...
    voice: {
      title: 'Voice fix',
      steps: ['Replace abstractions with the job: residents text, operators decide, and someone owns the next step.', 'Use first person when the point is personal and the claim is yours.', 'Cut anything that sounds like a press release or a transcript summary.'],
    },
    sources: {
      title: 'Recent links fix',
  ...
  const visualModes = [
    ...
    },
    {
      id: 'signal-flow',
      label: 'Signal flow',
      title: 'A resident message should not linger in the inbox.',
  ...
  const initSignalSim = () => {
    ...
    button.addEventListener('click', () => {
      ...
      output.innerHTML = `
        <strong>Resident messages for ${escapeHtml(label)}</strong>
